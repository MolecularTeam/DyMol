...
class Experience(object):
    ...
    def add_experience(self, experience):
        ...
        if len(self.memory)>self.max_size:
            ...
            self.memory.sort(key=lambda x: np.sum((x[1]+1e-10) * self.final_weight, axis=-1), reverse=True)
            self.memory = self.memory[:self.max_size]
            # print("\nBest score in memory: {:.2f}".format(self.memory[0][1]))

    def add_pareto_experience(self, experience, candidates):  # No length
        """Experience should be a list of (smiles, score, prior likelihood) tuples"""
        self.pareto_memory.extend(experience)
        idxs = []
        for p_smile in candidates:
            for i, exp in enumerate(self.pareto_memory):
                if p_smile == exp[0]:
                    idxs.append(i)
                    break
        self.pareto_memory = [self.pareto_memory[idx] for idx in idxs]

    def pareto_sample(self, n, weighted=False):
        """Sample a batch size n of experience"""
        if len(self.pareto_memory) < n:
            raise IndexError('Size of memory ({}) is less than requested sample ({})'.format(len(self), n))
        else:
            scores = [np.sum((x[1]+1e-10) * self.final_weight, axis=-1) for x in self.pareto_memory]
            if weighted:
                sample = np.random.choice(len(self.pareto_memory), size=n, replace=False, p=scores / np.sum(scores))
            else:
                sample = np.random.choice(len(self.pareto_memory), size=n, replace=False)
            sample = [self.pareto_memory[i] for i in sample]
            smiles = [x[0] for x in sample]
            scores = [np.sum(x[1] * self.final_weight, axis=-1) for x in sample]
    ...
    def sample(self, n, weighted=True):
        """Sample a batch size n of experience"""
        if len(self.memory)<n:
            raise IndexError('Size of memory ({}) is less than requested sample ({})'.format(len(self), n))
        else:
            scores = [np.sum((x[1]+1e-10) * (self.final_weight+self.weight_list), axis=-1) for x in self.memory]  # final + weight 변경?
            if weighted:
            ...
            smiles = [x[0] for x in sample]
            scores = [np.sum(x[1] * self.weight_list, axis=-1) for x in sample]
            prior_likelihood = [x[2] for x in sample]
        tokenized = [self.voc.tokenize(smile) for smile in smiles]
        encoded = [Variable(self.voc.encode(tokenized_i)) for tokenized_i in tokenized]
        encoded = MolData.collate_fn(encoded)
    ...
    def pareto_niche_sample(self, n, weighted=False):
        """Sample a batch size n of experience"""
        if len(self.memory) < n:
            raise IndexError('Size of memory ({}) is less than requested sample ({})'.format(len(self), n))
        else:
            ideal = np.array([0] * len(self.final_weight))
            nadir = np.array([1] * len(self.final_weight))
            F = np.array([1-x[1] for x in self.pareto_memory + self.memory])
            # niche_of_individuals is selected ref_dir index (num x)
            niche_of_individuals, dist_to_niche, dist_matrix = associate_to_niches(F, self.ref_dirs, ideal, nadir)
            selected_niche = np.unique(niche_of_individuals)
            # dist_matrix (num x, num ref_dir)
            if len(selected_niche) <= n:
                n = len(selected_niche)
            sampled_niche = np.random.choice(selected_niche, size=n, replace=False)
            sample = []
            for niche in sampled_niche:
                candi_idx = []
                for idx, indi in enumerate(niche_of_individuals):
                    if indi == niche:
                        candi_idx.append(idx)
                sample.append(np.random.choice(candi_idx, size=1))
    ...
    def sample_with_pref(self, n):
        ...
        if len(self.memory)<n:
            ...
            prior_likelihood = [x[2] for x in sample]
            prefs = [x[3] for x in sample]
        tokenized = [self.voc.tokenize(smile) for smile in smiles]
        encoded = [Variable(self.voc.encode(tokenized_i)) for tokenized_i in tokenized]
        encoded = MolData.collate_fn(encoded)
        return encoded, np.array(scores), np.array(prior_likelihood), np.array(prefs)
